pub static KEYS: Lazy::new(identity::Keypair::generate_ed25519);
...
impl AppBehaviour {
    pub async fn new(
        ...
        behaviour.floodsub.subsribe(CHAIN_TOPIC.clone());
        behaviour.floodsub.subsribe(BLOCK_TOPIC.clone());
...
impl NetworkBehaviourEventProcess<MdnsEvent> for AppBehaviour {
    fn inject_event(&mut self, event: MdnsEvent){
        match event {
            ...
            MdnsEvent::Discovered(expired_list) => {
...
impl NetworkBehaviourEventProcess for AppBehaviour {
    fn inject_event(&mut self, event: FloodsubEvent) {
        if let Ok(resp) = serde_json::from_slice::(&msg.data){
            ...
            let peer_id = resp.from_peeer_id;
